...
import main.java.br.com.rpires.domain.Produto3;
...
public class Venda implements Persistence {
	...
	public void adicionarProduto(Produto3 produto, Integer quantidade) {
		validarStatus();
	...
	// Método para validar se é possível alterar o status da venda
	...
	public void removerProduto(Produto3 produto, Integer quantidade) {
		validarStatus();
		Optional<ProdutoQuantidade> op = produtos.stream()
				.filter(filter -> filter.getProduto().getCodigo().equals(produto.getCodigo())).findAny();
